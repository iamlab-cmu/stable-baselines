...
from stable_baselines_custom import logger
from stable_baselines_custom.common import explained_variance, ActorCriticRLModel, tf_util, SetVerbosity, TensorboardWriter
from stable_baselines_custom.common.runners import AbstractEnvRunner
from stable_baselines_custom.common.policies import ActorCriticPolicy, RecurrentActorCriticPolicy
from stable_baselines_custom.common.schedules import get_schedule_fn
from stable_baselines_custom.common.tf_util import total_episode_reward_logger
from stable_baselines_custom.common.math_util import safe_mean
...
class PPO2(ActorCriticRLModel):
    ...
    def learn(self, total_timesteps, callback=None, log_interval=1, tb_log_name="PPO2",
              reset_num_timesteps=True, index_to_learn=None, all_models = None, tf_writer = None):
        # Transform to callable if needed
        self.learning_rate = get_schedule_fn(self.learning_rate)
        self.cliprange = get_schedule_fn(self.cliprange)
        cliprange_vf = get_schedule_fn(self.cliprange_vf)
        sv = SetVerbosity(self.verbose)
        sv.__enter__()
        if tf_writer is None:
            new_tb_log = self._init_num_timesteps(reset_num_timesteps)
            callback = self._init_callback(callback)
            writer = TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log).__enter__()
        else:
            writer = tf_writer
        self._setup_learn()
